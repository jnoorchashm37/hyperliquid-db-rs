...
| Node directory | Role |
| --- | --- |
| `periodic_abci_states` | Canonical HyperCore state snapshots every 10,000 blocks. Use to seed state-derived endpoints. Translate with `hl-node translate-abci-state`, or compute L4 books with `hl-node compute-l4-snapshots`. |
| `replica_cmds` | Accepted L1 transaction blocks. Use to replay state forward from a snapshot when exact live state is required. |
...
| --- | --- | --- | --- |
| `allMids` | `AllMids` | `periodic_abci_states` plus `node_raw_book_diffs_streaming`; fallback from `node_trades_streaming` or `node_fills_streaming` | Seed books from an L4 snapshot computed from ABCI state, apply raw book diffs, then publish the mid for each coin. If a book is empty, use the last trade price fallback. Spot mids are included only for the first perp dex. |
...
| `webData3` | `WebData3` | `periodic_abci_states` plus `replica_cmds` | State-derived aggregate. Requires user state plus per-dex state such as vault equity, OI-cap data, leading vaults, agent info, and server time. For exact live output, seed from ABCI state and replay `replica_cmds`; projection streams alone are incomplete. |
| `twapStates` | `TwapStates` | `periodic_abci_states` plus `node_twap_statuses_streaming` | Seed active TWAPs from state, then apply TWAP status events. Filter by `user` and `dex`. |
| `clearinghouseState` | `ClearinghouseState` | `periodic_abci_states` plus `replica_cmds` | Per-user perps account state: positions, margin summaries, maintenance margin, withdrawable. Exact live construction requires state replay. Projection streams like fills, funding/ledger events, and order events can help build a local cache, but they are not a complete substitute for the state transition logic. |
| `openOrders` | `OpenOrders` | `periodic_abci_states` or computed L4 snapshot, plus `node_order_statuses_streaming` and `node_raw_book_diffs_streaming` | Seed user orders from the state/L4 snapshot. Apply order statuses for lifecycle changes. Apply raw book diffs for exact remaining sizes and book-side changes. |
...
| `l2Book` | `WsBook` | `periodic_abci_states` computed L4 snapshot plus `node_raw_book_diffs_streaming` | Maintain full book by order, aggregate by price level, then emit top levels. Implement `nSigFigs` and `mantissa` aggregation at the output layer. |
...
| `userFundings` | `WsUserFundings` | `misc_events_streaming` | Funding payments are misc/ledger events. Filter by user when the event schema includes users; otherwise derive by replaying state/ledger transitions from `periodic_abci_states` plus `replica_cmds`. |
...
| `activeAssetCtx` | `WsActiveAssetCtx` / `WsActiveSpotAssetCtx` | `periodic_abci_states` plus `replica_cmds`; also `node_raw_book_diffs_streaming`, `node_trades_streaming`, `hip3_oracle_updates_streaming` | Asset-context fields are mixed. Mark/oracle/funding/OI/circulating supply are state-derived. `midPx` comes from the maintained book. Rolling day volume and previous-day price can be maintained from trades if not taken from state. HIP-3 oracle updates need `hip3_oracle_updates_streaming` or full state replay. |
| `activeAssetData` | `WsActiveAssetData` | `periodic_abci_states` plus `replica_cmds` | User plus asset risk state: leverage, max trade sizes, and available-to-trade. Exact live output is state-derived and should be produced from replayed state, not just fills/orders. |
...
| `bbo` | `WsBbo` | `periodic_abci_states` computed L4 snapshot plus `node_raw_book_diffs_streaming` | Same maintained book as `l2Book`, but emit only best bid and best ask when they change on a block. |
| `spotState` | `WsSpotState` | `periodic_abci_states` plus `replica_cmds`; projection helpers `node_fills_streaming` and `misc_events_streaming` | Spot balances and holds are state-derived. Spot fills and spot ledger transfers can update a cache, but exact live state should come from ABCI state plus replay. |
| `allDexsClearinghouseState` | `WsAllDexsClearinghouseState` | `periodic_abci_states` plus `replica_cmds` | Same as `clearinghouseState`, but compute for every perp dex for the user. |
| `allDexsAssetCtxs` | `WsAllDexsAssetCtxs` | `periodic_abci_states` plus `replica_cmds`; also `node_raw_book_diffs_streaming`, `node_trades_streaming`, `hip3_oracle_updates_streaming` | Same components as `activeAssetCtx`, but emit all perp dex asset contexts. |
...
2. For book-derived market channels, compute an L4 snapshot from the latest
   `periodic_abci_states/{date}/{height}.rmp`, then apply
   `node_raw_book_diffs_streaming` from that height forward.

3. For state-derived user or asset channels, use `periodic_abci_states` as the
   snapshot and replay `replica_cmds` for exact live state. Projection streams
   are useful for targeted caches, but they do not encode every state transition
   needed for exact `clearinghouseState`, `spotState`, `activeAssetData`, or
   `webData3`.
